...
<form action="ajblog.php?miss=<?php if(isset($_GET['miss'])){echo $_GET['miss'];}?>" method="POST" enctype="multipart/form-data">
...
if(isset($_POST['user'])&&isset($_POST['pass'])){//verification du uset et du pass
        try{
            ...
            $sql = "SELECT user, pass FROM blog WHERE user like :user AND pass LIKE :pass";
...
if(isset($_POST['user'])){
    if($passed==true){
        ...
        header('location:ajblog.php?miss='.($_GET['miss']+=1));
    }
}
if(isset($_FILES['photo'])){
        if ($_FILES['photo']['error']) {
            ...
            if ((isset($_FILES['photo']['name'])&&($_FILES['photo']['error'] == UPLOAD_ERR_OK))) {//renomage et déplacement du temp ver le dossier des images
                ...
                echo "<img src='./pho/".$_POST['title'].'.jpg'."' alt='error' ><br>";
            }
            else {
