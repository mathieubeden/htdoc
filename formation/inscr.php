...
<body>
  <form action="verific.php?ok=<?php if (isset($_GET['ok'])) {
                                  echo $_GET['ok'];
                                } ?>" method="post">
  ...
  <h3>
    <?php
    if (isset($_GET['miss'])) { //verifi les gets et te souhaite la bienvenu ou une érreur
      if ($_GET['miss'] == 2) {
        if (isset($_GET['id'])) {
          echo "bienvenu. vous avez été corectement enregistré avec l'id " . $_GET['id'] . ".";
        }
      } else {
        echo "des champs sont invalide ou manquant";
      }
    }
    ?>
  </h3>
